...
from simulator.utils.logger_config import ConsoleColor, setup_logger, update_logger_file
...
class SimulatorExecutor:
    ...
    def __init__(self, config: dict, output_path: str):
        ...
        if description_generator_path is None:
            logger.info(
                f"{ConsoleColor.CYAN}Start Building the policies graph:{ConsoleColor.RESET}"
            )
            descriptions_generator = DescriptionGenerator(
                environment=self.environment, config=config["description_generator"]
            )
            descriptions_generator.generate_policies_graph()
            logger.info(
                f"{ConsoleColor.CYAN}Finish Building the policies graph{ConsoleColor.RESET}"
            )
            pickle.dump(
                descriptions_generator,
                open(
                    os.path.join(
                        output_path, "policies_graph", "descriptions_generator.pickle"
                    ),
                    "wb",
                ),
            )
        ...
        event_generator = EventsGenerator(
            config=config["event_generator"], env=self.environment
        )
        self.dialog_manager = DialogManager(
            config["dialog_manager"], environment=self.environment
        )
    ...
    def run_simulation(self, experiment_name=""):
        ...
        if len(self.dataset_handler) == 0:
            print(
                f"{ConsoleColor.BLUE}The dataset is empty. Loading the last dataset...{ConsoleColor.RESET}"
            )
            self.load_dataset()
        ...
        json.dump(
            self.dataset_handler.descriptions_generator.policies,
        ...
        if num_records == 0:
            logger.warning(
                f"{ConsoleColor.RED}No records found to process.{ConsoleColor.RESET}"
            )
        ...
        logger.info(
            f"{ConsoleColor.CYAN}Start running the simulator{ConsoleColor.RESET}"
        )
        intermediate_res = os.path.join(experiment_dir, "res_dump.pickle")
        if os.path.isfile(intermediate_res):
            all_res, start_iteration, total_cost = pickle.load(
                open(intermediate_res, "rb")
            )
        # Handle batches
        for i in range(start_iteration, num_batch, 1):
            ...
            res, cost = self.dialog_manager.run_events(
                records[i * mini_batch_size : (i + 1) * mini_batch_size]
            )
            all_res.extend(res)
            total_cost += cost
        ...
        if remaining_records:
            logger.info(
                f"{ConsoleColor.WHITE}Running remaining records...{ConsoleColor.RESET}"
            )
            if total_cost <= self.config["dialog_manager"]["cost_limit"]:
        ...
        logger.info(
            f"{ConsoleColor.CYAN}Finish running the simulator{ConsoleColor.RESET}"
        )
        track_event(
            RunSimulationEvent(
                cost=total_cost,
                ...
                avg_n_user_messages_per_dialog=sum(
                    len(entry["res"]["user_messages"]) for entry in all_res
                )
                / len(all_res)
                if all_res
                else 0,
                avg_n_chatbot_messages_per_dialog=sum(
                    len(entry["res"]["chatbot_messages"]) for entry in all_res
                )
                / len(all_res)
                if all_res
                else 0,
    ...
    def analyze_results(self, results, experiment_dir):
        ...
        """
        results = get_dialog_policies(
            self.config["analysis"], results, self.dataset_handler.records
        )
        all_rows = []
        valid_event_ind = []
        for r in results:
            try:
                ...
                cur_row = {
                    ...
                    if "user_thoughts" in r["res"]
                    else None,
                    "scenario": getattr(cur_event, "scenario", None),
                    "expected_behaviour": getattr(
                        cur_event.description, "expected_behaviour", None
                    ),
                    "challenge_level": getattr(
                        cur_event.description, "challenge_level", None
                    ),
                ...
                track_event(
                    ExceptionEvent(
                        exception_type=type(e).__name__, error_message=error_message
                    )
                )
                continue
        non_valid_rows = []
        for i, event in enumerate(self.dataset_handler.records):
            ...
            cur_row = {
                "id": i + 1,
                "score": 0,
                "challenge_level": getattr(event.description, "challenge_level", None),
            }
        ...
        if all_rows:
            ...
            track_event(AnalyzeSimulationResultsEvent(failure_rate=failure_rate))
            logger.info(
                f"{ConsoleColor.CYAN}Finish running results analysis{ConsoleColor.RESET}"
            )
        else:
            logger.info(
                f"{ConsoleColor.CYAN}No rows to process. Results are empty.{ConsoleColor.RESET}"
            )
    ...
    def set_output_folder(output_path):
        ...
        if os.path.isfile(
            os.path.join(output_path, "policies_graph", "descriptions_generator.pickle")
        ):
            description_generator_path = os.path.join(
                output_path, "policies_graph", "descriptions_generator.pickle"
            )

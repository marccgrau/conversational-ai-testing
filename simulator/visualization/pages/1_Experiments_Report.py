...
st.set_page_config(
    page_title="Experiments report", page_icon=base_path + "images/hsg-logo.png"
)
...
def _format_arrow(val):
    if pd.isna(val):
        return val
    return (
        f"{abs(val):.0f}%" if val == 0 else f"{'↑' if val > 0 else '↓'} {abs(val):.0f}%"
    )
...
def extract_violated_policies_str(row):
    ...
    try:
        ...
        violated_policies = [
            policies[j]["flow"] + ": " + policies[j]["policy"]
            for j in violated_policies_ind
        ]
...
def read_experiment_data(exp_path: str):
    ...
    for flow, policies in policies_info.items():
        for policy in policies:
            policies_info_list.append(
                {"name": flow + ": " + policy["policy"], "category": policy["category"]}
            )
    all_policies_list = []
    for i, row in df.iterrows():
        ...
        for j in policies_sublist:
            ...
            all_policies_list.append(
                {
                    "policy": policies[j]["flow"] + ": " + policies[j]["policy"],
                    "score": score,
    ...
    for policy_info in policies_info_list:
        cur_policies = [
            ...
            if policy["policy"] == policy_info["name"]
        ]
        if len(cur_policies) < 3:
            success_rate.append(-1)
            continue
        success_rate.append(
            100 * sum([policy["score"] for policy in cur_policies]) / len(cur_policies)
        )
    graph_info = {"scores": scores, "challenge_level": challenge}
    table_policies_info = {
        "policy": [policy["name"] for policy in policies_info_list],
...
def load_data(database_path=None):
    ...
    for exp in experiments_list:
        ...
        events_info = events_info.rename(
            columns={"violated_policies": f"{exp_name}_violated_policies"}
        )
        if events_df is None:
            events_df = events_info
        else:
            events_df = pd.merge(
                events_df, events_info, on=["id", "scenario"], how="outer"
            )

        policies_datasets.append(
            pd.DataFrame(
                {
                    "policy": table_policies_info["policy"],
    ...
    mean_scores = merged_df[score_columns].apply(
    ...
    for i in range(1, len(score_columns)):
        ...
        merged_df[new_col_name] = merged_df.apply(
            lambda row: None
            if row[current_col] == -1 or row[prev_col] == -1
            else row[current_col] - row[prev_col],
            axis=1,
        )
...
def main():
    ...
    st.sidebar.text_input(
        "Database path", key="database_path", on_change=change_data, value=last_db_path
    )
    database_path = st.session_state.get("database_path", None)
    data, policies_df, styled_col, events_df = load_data(database_path)
    if data.empty:
        st.write(
            "The database you selected does not contain any experiments. Please select another database."
        )
        return
    ...
    if experiments:
        ...
        valid_columns = ["category"] + [
            col
            for col in policies_df.columns
            if any(expr in col for expr in unique_exp)
        ]
        ...
        filtered_df = filtered_df.style.format(
            _format_arrow, subset=cur_styled_col
        ).map(_color_arrow, subset=cur_styled_col)

        score_columns_filter = [
            col
            for col in events_df.columns
            if "score" in col and any(expr in col for expr in unique_exp)
        ]

        exp_columns = [
            col
            for col in events_df.columns
            if ("_score" in col) or ("_reason" in col) or ("_violated_policies" in col)
        ]
        exp_columns_filter = [
            col for col in exp_columns if any(expr in col for expr in unique_exp)
        ]
        cur_events_df = events_df.drop(
            columns=[col for col in exp_columns if col not in exp_columns_filter]
        )
        cur_events_df = cur_events_df.style.format(
            _format_binary, subset=score_columns_filter
        ).map(_color_binary, subset=score_columns_filter)
        ...
        st.dataframe(cur_events_df, use_container_width=True)
    else:

...
def add_dataframe(self, df):
    ...
    self.log_messages.append(
        f"<div style='color:lightgreen;'>{html_table}</div>"
...
svg_base64_sprechblase = get_base64_svg(
    base_path + "images/hsg-icon-user-sprechblase.svg"
)
svg_base64_lupe = get_base64_svg(base_path + "images/hsg-icon-lupe.svg")
...
def on_select_thread():
    ...
    thread_id = st.session_state["threads"][
        st.session_state["event_id"].index(event_id)
    ]
    ...
    try:
        with col2:
            cursor.execute(
                "SELECT * FROM Dialog WHERE thread_id = ? ORDER BY time ASC",
                (thread_id,),
            )
        ...
        with col1:
            cursor.execute(
                "SELECT * FROM Tools WHERE thread_id = ? ORDER BY time ASC",
                (thread_id,),
            )
            rows = cursor.fetchall()
            for row in rows:
                ...
                if "Error:" in row[3]:
                    logger_chat.log_message(
                        f"Response:<br>{row[3]}<br>----------<br>", "error"
                    )
                else:
                    logger_chat.log_message(
                        f"Response:<br>{row[3]}<br>----------<br>", "warning"
                    )
        ...
        with col3:
            cursor.execute(
                "SELECT * FROM Thoughts WHERE thread_id = ? ORDER BY time ASC",
                (thread_id,),
            )
            rows = cursor.fetchall()
            for row in rows:
...
st.set_page_config(
    page_title="Session vizualization",
...
with col2:
    st.markdown(
        "<h1 style='text-align: center;'>DialogDojo</h1>", unsafe_allow_html=True
    )
...
with col1:
    st.markdown(
        ...
        ChatBot logging
        <img src="data:image/svg+xml;base64,{svg_base64_lupe}"
            alt="log icon"
            style="height: 30px; vertical-align: middle; margin-left: 5px;" />
        </h1>
        """,
        unsafe_allow_html=True,
    )
...
with col3:
    st.markdown(
        f"""
        <h1 style='font-size: 20px;'>User thoughts 
        <img src="data:image/svg+xml;base64,{svg_base64_sprechblase}"
            alt="log icon"
            style="height: 30px; vertical-align: middle; margin-left: 5px;" />
        </h1>
        """,
...
def main():
    ...
    if "last_db_path" not in st.session_state:
        ...
        st.session_state["event_id"], st.session_state["threads"] = extract_threads(
            last_db_path
        )
    ...
    st.sidebar.text_input(
        "Memory path",
        key="memory_path",
        on_change=update_thread_list,
        value=st.session_state.last_db_path,
    )

    st.sidebar.selectbox(
        "Select an event to visualized:",
        st.session_state["event_id"],
        key="selected_event",

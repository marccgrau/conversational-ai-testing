...
from simulator.utils.llm_utils import get_llm, set_callback, set_llm_chain
...
class Policy(BaseModel):
    ...
    challenge_score: int = Field(
        description="The challenge score of the policy between 1-5"
    )
...
class EventDescription(BaseModel):
    ...
    expected_behaviour: str = Field(
        description="The expected behaviour of the chatbot according to the policies"
    )
...
class DescriptionGenerator:
    ...
    def __init__(self, config: dict, environment: Env):
        ...
        self.llm_description = set_llm_chain(
            llm,
            structure=EventDescription,
        ...
        if self.config["refinement_config"]["do_refinement"]:
            ...
            self.feedback_chain = set_llm_chain(
                llm, **self.config["refinement_config"]["prompt_feedback"]
            )
            self.refinement_chain = set_llm_chain(
                llm, **self.config["refinement_config"]["prompt_refinement"]
            )
    ...
    def generate_policies_graph(self, override=False):
        ...
        if override or not hasattr(self, "flows"):
            logger.info(
                f"{ConsoleColor.WHITE}Step 1: Breaking prompt to flows{ConsoleColor.RESET}"
            )
        ...
        if override or not hasattr(self, "policies"):
            logger.info(
                f"{ConsoleColor.WHITE}Step 2: Breaking each flow to policies{ConsoleColor.RESET}"
            )
        ...
        if override or not hasattr(self, "relations"):
            logger.info(
                f"{ConsoleColor.WHITE}Step 3: Building the relations graph{ConsoleColor.RESET}"
            )
            self.extract_graph()
    ...
    def extract_flows(self):
        ...
        """
        if (
            len(self.prompt.split(" ")) < 300
        ):  # Very short prompt, no need to split to flows. TODO: Change magic number
            return ["Conversation with the chatbot"]
        llm = get_llm(self.config["llm_policy"])
        flow_extractor = set_llm_chain(
            llm, structure=FlowsList, **self.config["flow_config"]["prompt"]
        )
        result = batch_invoke(
            flow_extractor.invoke,
    ...
    def extract_policies(self):
        ...
        """
        llm = get_llm(self.config["llm_policy"])
        policy_extractor = set_llm_chain(
            llm, **self.config["policies_config"]["prompt"], structure=PoliciesList
        )
        flows_policies = {}
        batch = []
        ...
        for i, result in enumerate(res):
            if result["error"] is not None:
                ...
                batch_error_message = (
                    batch_error_message or ""
                ) + f"flow {result['index']}: {result['error']} "
                continue
    ...
    def extract_graph(self):
        ...
        edge_llm = set_llm_chain(
            llm, structure=Rank, **self.config["edge_config"]["prompt"]
        )
        ...
        for i, first_policy in enumerate(policies_list):
            for j, second_policy in enumerate(policies_list[i + 1 :]):
                samples_batch.append(
                    {
                        "policy1": policy_to_str(first_policy),
        ...
        for result in res:
            if result["error"] is not None:
                ...
                batch_error_message = (
                    batch_error_message or ""
                ) + f"edge {result['index']}: {result['error']} "
                continue
        ...
        std_edge_weight = math.sqrt(
            sum((edge[2]["weight"] - avg_edge_weight) ** 2 for edge in all_edges)
            / n_edges
        )
        self.graph_info["G"].add_edges_from(all_edges)
        track_event(
            GenerateRelationsGraphEvent(
                cost=graph_creation_cost,
                n_edges=n_edges,
    ...
    def sample_from_graph(self, threshold) -> Tuple[list, int]:
        ...
        while path_sum < threshold:
            ...
            weights = [
                self.graph_info["G"][current_node][neighbor]["weight"]
    ...
    def sample_description(
        self, challenge_complexity: int or list[int], num_samples: int = 1
    ) -> Tuple[list[Description], float]:
        """
        ...
        for result in res:
            if result["error"] is not None:
                continue
            cost += result["usage"]
            all_policies[result["index"]]["description"] = result[
                "result"
            ].event_description
            all_policies[result["index"]]["expected_behaviour"] = result[
                "result"
            ].expected_behaviour
        all_policies = [policy for policy in all_policies if "description" in policy]
        descriptions = [
            Description(
                event_description=policy["description"],
                expected_behaviour=policy["expected_behaviour"],
                policies=policy["policies"],
                challenge_level=policy["path_sum"],
            )
            for policy in all_policies
        ]
        refinement_cost = 0
        if self.config["refinement_config"]["do_refinement"]:
            descriptions, refinement_cost = self.expected_behaviour_refinement(
                descriptions
            )
        cost += refinement_cost
        return descriptions, cost

    def expected_behaviour_refinement(
        self, descriptions: list[Description], num_iterations=1
    ) -> Tuple[list[Description], float]:
        """
        ...
        for i in range(num_iterations):
            ...
            for ind in iteration_indices:
                batch_input.append(
                    {
                        "description": descriptions[ind].event_description,
                        "behaviour": descriptions[ind].expected_behaviour,
                        "prompt": self.prompt,
                    }
                )
            res = async_batch_invoke(
                self.feedback_chain.ainvoke,
                batch_input,
                num_workers=num_workers,
                callbacks=[callback],
                timeout=timeout,
            ...
            for j, result in enumerate(res):
                if result["error"] is not None or "None" in result["result"].content:
                    ...
                    descriptions[
                        cur_refine_indices[result["index"]]
                    ].expected_behaviour = result["result"].content
                    cost += result["usage"]

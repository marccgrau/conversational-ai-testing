...
from simulator.utils.llm_utils import dict_to_str, get_llm, set_callback, set_llm_chain
...
class EventsGenerator:
    ...
    def __init__(self, config: dict, env: Env):
        ...
        self.env = env
        self.init_agent()
        self.llm_symbolic = set_llm_chain(
            self.llm,
            **config["symbolic_enrichment_config"]["prompt"],
            structure=info_symbolic,
        )
        self.llm_constraints = set_llm_chain(
            self.llm, **config["symbolic_constraints_config"]["prompt"]
        )
    ...
    def init_executors(self) -> dict[AgentTools]:
        ...
        for table_name, example in rows_data.items():
            ...
            system_messages = hub.pull(
                self.config["event_graph"]["prompt_executors"]["prompt_hub_name"]
            )
            system_messages = system_messages.partial(
                schema=self.env.data_schema[table_name],
                example=json.dumps(rows_data[table_name]),
            )
            agent_executor = AgentTools(
                llm=self.llm,
                tools=[think, table_insertion_tools[table_name]],
    ...
    def get_insertion_function(self, table_name: str):
        def tool_function(
            json_row: str, dataset: Annotated[dict, InjectedState("dataset")]
        ):
            try:
                ...
                dataset[table_name] = pd.concat(
                    [dataset[table_name], df], ignore_index=True
                )
    ...
    def init_agent(self):
        ...
        """
        llm_filter_restrictions = set_llm_chain(
            self.llm, **self.config["event_graph"]["prompt_restrictions"]
        )
        llm_final_res = set_llm_chain(
            self.llm,
            **self.config["event_graph"]["prompt_final_res"],
            structure=FinalResult,
        )
        self.agent = EventGraph(
            executors=self.init_executors(),
    ...
    def symbolic_to_event(self, symbolic_event: EventSymbolic) -> Event:
        ...
        if "## Rows Constraints:\n" in symbolic_event.policies_constraints:
            policies_constraints = symbolic_event.policies_constraints.split(
                "## Rows Constraints:\n"
            )[1]
        else:
            policies_constraints = ""
        res = self.agent.invoke(
            rows_to_generate=event_dict["tables_rows"],
            rows_generated=[],
            event_description=event_dict["enriched_scenario"],
        ...
        event = Event(
            description=symbolic_event.description,
            database=res["dataset"],
            scenario=res["final_response_scenario"],
    ...
    def symbolics_to_events(
        self, symbolic_events: list[EventSymbolic]
    ...
    def descriptions_to_symbolic(
        self, descriptions: list[Description]
    ) -> tuple[list[EventSymbolic], float]:
        """
        ...
        for i, description in enumerate(descriptions):
            samples_batch.append(
                {"tables_info": schema, "scenario": description.event_description}
            )
        ...
        for result in res:
            if result["error"] is not None:
                continue
            cost += result["usage"]
            cur_event = EventSymbolic(
                symbolic_info=result["result"],
    ...
    def get_symbolic_constraints(
        self, events: list[EventSymbolic]
    ) -> Tuple[list[EventSymbolic], float]:
        """
        ...
        for i, event in enumerate(events):
            samples_batch.append(
                {"symbolic_info": str(event), "system_prompt": self.env.prompt}
            )
